...
func TestApplyScaleSubresource(t *testing.T) {
	...
	// TODO: We actually also own ".spec" so this isn't going to work, but we can assert something more accurate here.
	if got := len(obj.GetManagedFields()); got != 1 {
		t.Fatalf("Expected 1 managed fields, got %v: %v", got, obj.GetManagedFields())
	}
}
